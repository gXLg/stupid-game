...
try :
  server_sock = Listener ( ( 'localhost', int ( argv [ 1 ])))
except :
...
while run :
  ...
  if player2.cooldown <= 0 : run = False
  if keys [ pygame.K_q ] : run = False
  if not frame % 4 :
    for i in entities + [ player, player2 ] : i.dmg ( )
    for i in entities : i.attack ( )

  player.move ( controller = True )
  if not frame % 2 :
...
draw_game ( )
pygame.display.flip ( )
print ( "You - Kills:", kills, "Health:", player.cooldown )
print ( "Opponent - Kills:", kills2, "Health:", player2.cooldown )
pygame.quit ( )
exit ( )
